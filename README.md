...
- Patrick
  - Added /src folder for source files
  - Implemented a first draft of the Graph class with basic functions like adding new edges, finding edges, and getting neighbors of a node (needed for traversal)
  - Have not done any thorough testing yet, so there are likely some bugs
...
If you've made a mistake (_happens to the best of us_) in the changes you've made and committed, pushed, or merged, you can undo the majority of those changes using Git CLI. This [guide](https://sethrobertson.github.io/GitFixUm/fixup.html) allows you to interactively navigate through the resources based on the actions you've taken to find the solution for your problem.
